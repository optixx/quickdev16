...
static
BOOL lock_fs(FATFS * fs         /* File system object */
    )
{
    ...
    switch (fs->fs_type) {
        ...
        case FS_FAT32:
            if (move_window(fs, fsect + (clst / (SS(fs) / 4))))
                break;
            return LD_DWORD(&fs->
                            win[((WORD) clst * 4) & (SS(fs) - 1)]) & 0x0FFFFFFF;
    }
...
#if !_FS_READONLY
static
FRESULT put_cluster(FATFS * fs, /* File system object */
...
#if !_FS_READONLY
static
FRESULT remove_chain(FATFS * fs,        /* File system object */
...
static
FRESULT dir_seek(DIR * dj,      /* Pointer to directory object */
    ...
    )
{
    ...
    do {
        ...
        wc2 = lfnbuf[i++];
        if (IsLower(wc1))
            wc1 -= 0x20;        /* Compare it (ignore case) */
        if (IsLower(wc2))
            wc2 -= 0x20;
        if (wc1 != wc2)
            return FALSE;
    ...
    )
{
    ...
    if (dir[LDIR_Ord] & 0x40)
        lfnbuf[i] = 0;          /* Put terminator if last LFN entry */
    ...
    )
{
    ...
    do {
        if (wchr != 0xFFFF)
            wchr = lfnbuf[i++]; /* Get an effective char */
        ST_WORD(dir + LfnOfs[s], wchr); /* Put it */
        if (!wchr)
            wchr = 0xFFFF;      /* Padding chars following last char */
    } while (++s < 13);
    if (wchr == 0xFFFF || !lfnbuf[i])
...
#if _USE_LFN
static
BYTE sum_sfn(const BYTE * dir   /* Ptr to directory entry */
    )
{
    BYTE sum = 0;
    int n = 11;
...
static
FRESULT dir_find(DIR * dj       /* Pointer to the directory object linked to the file name */
    )
{
    FRESULT res;
            ...
            if (a == AM_LFN) {  /* An LFN entry is found */
                if (dj->lfn) {
                    ...
                    ord = (c == ord && sum == dir[LDIR_Chksum]
                           && (!lfen
                               || test_lfn(dj->lfn, dir))) ? ord - 1 : 0xFF;
                }
...
#if _FS_MINIMIZE <= 2
static
FRESULT dir_read(DIR * dj       /* Pointer to the directory object to store read object name */
    )
{
    FRESULT res;
...
static
FRESULT create_name(DIR * dj,   /* Pointer to the directory object */
    ...
    )
{
    ...
    *path = &p[si];             /* Rerurn pointer to the next segment */
    ...
    for (;;) {
        ...
        if (w == 0)
            break;              /* Break when enf of the LFN */
        ...
        if (i >= ni || si == di) {      /* Here is extension or end of SFN */
            ...
            if (si != di)
                cf |= 1;        /* File name is longer than 8 bytes */
            if (si > di)
                break;          /* No extension */
            si = di;
            i = 8;
        ...
        if (w >= 0x80)
            cf |= 0x20;         /* If there is any extended char, force create an LFN */
    ...
    if ((cf & 0x21) == 0) {     /* When LFN is in 8.3 format without extended char, NT flags are created */
        if ((b & 0x03) == 0x01)
            cf |= 0x10;         /* NT flag (Extension has only small capital) */
        if ((b & 0x0C) == 0x04)
            cf |= 0x08;         /* NT flag (Filename has only small capital) */
        if ((b & 0x0C) != 0x0C && (b & 0x03) != 0x03)
            cf |= 2;            /* Eliminate LFN when non composite capitals */
    }
    ...
    for (;;) {
        ...
        if (c >= 0x80)
            b |= 3;             /* If there is any extended char, eliminate NT flag */
    ...
    for (; btr;                 /* Repeat until all data transferred */
        ...
        if ((fp->fptr % SS(fp->fs)) == 0) {     /* On the sector boundary? */
            ...
            if (fp->flag & FA__DIRTY) { /* Write sector I/O buffer if needed */
                if (disk_write(fp->fs->drive, fp->buf, fp->dsect, (BYTE) 1) !=
                    RES_OK)
                    ABORT(fp->fs, FR_DISK_ERR);
...
#if _FS_TINY
            if (fp->fs->winsect == fp->dsect && move_window(fp->fs, (DWORD) 0)) /* Write back data buffer prior to following direct
                                                                                 * transfer */
                ABORT(fp->fs, FR_DISK_ERR);
#else
...
FRESULT f_getfree(const char *path,     /* Pointer to the logical drive number (root dir) */
    ...
    )
{
    ...
    if (fat == FS_FAT12) {
        clst = 2;
        do {
            if ((WORD) get_cluster(*fatfs, clst) == 0)
                n++;
